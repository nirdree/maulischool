...
const INITIAL_DATA = {
  ...
  homework: [
    { _id: "hw1", classId: "cls3", subjectId: "sub1", teacherId: "emp1", title: "Chapter 5 Exercise", description: "Complete exercises 5.1 to 5.5 from textbook", dueDate: "2024-11-08", academicYear: "ay1", createdAt: "2024-11-04" },
    { _id: "hw2", classId: "cls3", subjectId: "sub2", teacherId: "emp2", title: "Essay Writing", description: "Write a 200-word essay on your favourite festival", dueDate: "2024-11-10", academicYear: "ay1", createdAt: "2024-11-05" },
    { _id: "hw3", classId: "cls3", subjectId: "sub3", teacherId: "emp3", title: "Draw Plant Cell", description: "Draw and label a plant cell diagram", dueDate: "2024-11-12", academicYear: "ay1", createdAt: "2024-11-05" },
  ],
  notices: [
    { _id: "ntc1", title: "Annual Sports Day", content: "Annual Sports Day will be held on 25th November 2024. All students must participate. Parents are cordially invited.", targetRoles: ["parent", "teacher"], priority: "Important", publishDate: "2024-11-01", expiryDate: "2024-11-26", createdBy: "usr2", academicYear: "ay1" },
    { _id: "ntc2", title: "Staff Meeting", content: "All teaching staff must attend the staff meeting on 15th November 2024 at 3:00 PM in the conference room.", targetRoles: ["teacher"], priority: "Urgent", publishDate: "2024-11-04", expiryDate: "2024-11-15", createdBy: "usr2", academicYear: "ay1" },
    { _id: "ntc3", title: "Fee Reminder", content: "Kindly note that November fees are due by 10th November 2024. Late fees will attract a fine of ₹10 per day.", targetRoles: ["parent"], priority: "Normal", publishDate: "2024-11-01", expiryDate: "2024-11-30", createdBy: "usr1", academicYear: "ay1" },
  ],
  leaves: [
  ...
  schoolSettings: {
    ...
    website: "www.sunrise.edu",
    logo: null,
    affiliationNo: "CBSE-12345",
    board: "CBSE",
    declaration: "I hereby declare that all information provided in this admission form is true and correct to the best of my knowledge. I agree to abide by the rules and regulations of the school.",
    lateFinePer: 10,
    feeDueDay: 10,
    minAttendance: 75
  },
  ...
  <div className="fixed top-4 right-4 z-50 space-y-2">
    {toasts.map(t => (
      <div key={t.id} className={`flex items-center gap-2 px-4 py-3 rounded-lg shadow-lg text-white text-sm font-medium ${
        t.type === 'success' ? 'bg-emerald-500' : t.type === 'error' ? 'bg-red-500' : t.type === 'warning' ? 'bg-amber-500' : 'bg-blue-500'
      }`}>
    ...
    <table className="w-full text-sm">
      <thead>
        <tr className="border-b border-gray-100 bg-gray-50">
          {columns.map((col, i) => (
            <th key={i} className="text-left px-4 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wide whitespace-nowrap">{col.header}</th>
          ))}
        </tr>
      </thead>
      <tbody>
        {data.length === 0 ? (
          <tr><td colSpan={columns.length} className="px-4 py-12 text-center text-gray-400">
            <div className="flex flex-col items-center gap-2">
              ...
              <span className="font-medium">{emptyMsg}</span>
            </div>
          </td></tr>
        ) : data.map((row, i) => (
          <tr key={i} onClick={onRowClick ? () => onRowClick(row) : undefined} className={`border-b border-gray-50 hover:bg-gray-50 transition-colors ${onRowClick ? 'cursor-pointer' : ''}`}>
            {columns.map((col, j) => (
              <td key={j} className="px-4 py-3 whitespace-nowrap">{col.render ? col.render(row) : row[col.key]}</td>
            ))}
          </tr>
        ))}
      </tbody>
...
const Avatar = ({ name, photo, size = 8 }) => {
  ...
  return photo ? (
    <img src={photo} alt={name} className={`w-${size} h-${size} rounded-full object-cover`} />
  ) : (
    <div className={`w-${size} h-${size} rounded-full ${color} flex items-center justify-center text-white font-semibold text-xs`}>{initials}</div>
  );
};
  ...
  <div className="flex gap-1 p-1 bg-gray-100 rounded-xl overflow-x-auto">
    {tabs.map(t => (
      <button key={t.value} onClick={() => onChange(t.value)}
        className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-all ${activeTab === t.value ? 'bg-white shadow text-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}>
        {t.label}
      </button>
    ))}
  </div>
      ...
      <div className="flex items-center gap-1 text-xs text-gray-400 mb-2">
        {breadcrumbs.map((b, i) => (
          <span key={i} className="flex items-center gap-1">
            {i > 0 && <span>/</span>}
            <span className={i === breadcrumbs.length - 1 ? 'text-gray-600 font-medium' : 'hover:text-gray-600 cursor-pointer'}>{b}</span>
          </span>
        ))}
      </div>
    )}
      ...
      <div key={i} className="flex items-center flex-1 last:flex-none">
        <div className="flex flex-col items-center">
          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-all ${
            ...
            i === current ? 'border-indigo-600 bg-white text-indigo-600' :
            'border-gray-200 bg-white text-gray-400'
          }`}>{i < current ? '✓' : i + 1}</div>
          <span className={`text-xs mt-1 whitespace-nowrap hidden sm:block ${i === current ? 'text-indigo-600 font-medium' : 'text-gray-400'}`}>{step}</span>
        </div>
        {i < steps.length - 1 && <div className={`flex-1 h-0.5 mx-1 ${i < current ? 'bg-indigo-600' : 'bg-gray-200'}`} />}
      </div>
    ))}
  </div>
);
...
const NAV_CONFIG = {
  ...
  principal: [
    ...
    { section: "Academic", items: [
      { id: "employees", label: "Teachers", icon: "👨‍🏫" },
      { id: "classrooms", label: "Classrooms", icon: "🏫" },
      { id: "exams", label: "Exams & Marks", icon: "📝" },
      { id: "timetable", label: "Timetable", icon: "🗓️" },
    ]},
  ...
  teacher: [
    ...
    { section: "My Class", items: [
      ...
      { id: "student-attendance", label: "Mark Attendance", icon: "✅" },
  ...
  parent: [
    ...
    { section: "My Child", items: [
      ...
      { id: "student-attendance", label: "Attendance", icon: "✅" },
...
const LoginView = ({ onLogin }) => {
  ...
  const handleLogin = async (e) => {
    ...
    if (found && password === found.pass) {
      onLogin({ email, role: found.role, name: INITIAL_DATA.users.find(u => u.email === email)?.name || found.label });
    } else {
            ...
            <div className="grid grid-cols-2 gap-2">
              {DEMO_ACCOUNTS.map(acc => (
                <button key={acc.role} onClick={() => { setEmail(acc.email); setPassword(acc.pass); }}
                  className="text-xs p-2 rounded-lg border border-gray-200 hover:border-indigo-300 hover:bg-indigo-50 text-left transition-colors">
                  <span className="font-semibold text-gray-700">{acc.label}</span>
                  <br /><span className="text-gray-400">{acc.email}</span>
                </button>
              ))}
            </div>
          </div>
...
const PublicEnquiryView = ({ onBack }) => {
  const { data, setData, showToast, getClassrooms } = useApp();
  ...
  const handleSubmit = async () => {
    ...
    const newEnq = {
      _id: `enq${Date.now()}`, enquiryId: genId("ENQ", data.enquiries),
      ...form, status: "New", academicYear: "ay1", createdAt: new Date().toISOString(), convertedToAdmission: false
    };
    setData(d => ({ ...d, enquiries: [...d.enquiries, newEnq] }));
    setSubmitted(newEnq);
  ...
  if (submitted) return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-50 flex items-center justify-center p-4">
      <Card className="p-8 max-w-md w-full text-center">
        ...
        <p className="text-xs text-gray-400 mb-4">Please save this ID to check your enquiry status.</p>
        ...
        <Card className="p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              <Select label="Class Applying For" required value={form.classApplying} onChange={e => f('classApplying', e.target.value)} error={errors.classApplying}
                options={classes.map(c => ({ value: c._id, label: c.displayName }))} />
            </div>
            ...
            <div className="sm:col-span-2">
              <Textarea label="Residential Address" required value={form.residentialAddress} onChange={e => f('residentialAddress', e.target.value)} error={errors.residentialAddress} rows={2} />
            </div>
              ...
              <div className="flex gap-4 pt-1">
                {["Male","Female","Other"].map(g => (
                  <label key={g} className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="gender" value={g} checked={form.gender === g} onChange={() => f('gender', g)} className="text-indigo-600" />
            ...
            <div className="sm:col-span-2">
              <Textarea label="Remarks" value={form.remark} onChange={e => f('remark', e.target.value)} placeholder="Any additional information..." rows={2} />
            </div>
          </div>
...
const AdminDashboard = () => {
  ...
  const feeData = [
    ...
    { month: "Jul", collected: 92000, target: 100000 },
    { month: "Aug", collected: 88000, target: 100000 },
    { month: "Sep", collected: 95000, target: 100000 },
    { month: "Oct", collected: 90000, target: 100000 },
    { month: "Nov", collected: 42000, target: 100000 },
  ];
            ...
            {data.enquiries.slice(0, 4).map(enq => {
              ...
              return (
                <div key={enq._id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                  <div>
                    <p className="font-medium text-sm text-gray-900">{enq.childName}</p>
                    <p className="text-xs text-gray-500">{cls?.displayName} · {enq.enquiryId}</p>
                  </div>
                ...
                <div key={fee._id} className="flex items-center justify-between p-3 bg-red-50 rounded-xl">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">{cls?.displayName} · {MONTHS[fee.month - 1]}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-red-600 text-sm">{fmtCurrency(fee.finalAmount)}</p>
                ...
                <div key={leave._id} className="flex items-center justify-between p-3 bg-amber-50 rounded-xl">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">{leave.leaveType} · {leave.totalDays} day(s)</p>
                    </div>
                  </div>
...
const EnquiriesView = ({ userRole }) => {
  const { data, setData, getClassrooms, showToast } = useApp();
  ...
  const filtered = data.enquiries.filter(e => {
    ...
    return (!q || e.childName?.toLowerCase().includes(q) || e.enquiryId?.includes(q) || e.mobileNo?.includes(q) || e.email?.toLowerCase().includes(q)) &&
      (!statusFilter || e.status === statusFilter) &&
      (!classFilter || e.classApplying === classFilter);
  });

  const handleStatusChange = (enq, status) => {
    setData(d => ({ ...d, enquiries: d.enquiries.map(e => e._id === enq._id ? { ...e, status } : e) }));
  ...
  const columns = [
    ...
    { header: "Actions", render: r => (
      <div className="flex items-center gap-1">
        ...
        {r.status !== 'AdmissionDone' && (
          <button onClick={e => { e.stopPropagation(); handleGeneratePassword(r); }} className="p-1 text-emerald-600 hover:bg-emerald-50 rounded text-xs font-medium">🔑</button>
        )}
      </div>
    )},
  ];
  ...
  return (
    <div className="space-y-5">
      ...
      <Card>
        <Table columns={columns} data={filtered} emptyMsg="No enquiries found" />
      </Card>
        ...
        {editData && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <Select label="Class" value={editData.classApplying} onChange={e => setEditData(p => ({...p, classApplying: e.target.value}))}
                options={classes.map(c => ({value:c._id, label:c.displayName}))} />
            </div>
            ...
            <div className="flex gap-3 justify-end">
              ...
              <Button onClick={() => {
                setData(d => ({ ...d, enquiries: d.enquiries.map(e => e._id === editData._id ? editData : e) }));
                showToast("Enquiry updated!", "success"); setShowModal(false);
              }}>Save Changes</Button>
            </div>
          </div>
        )}
      ...
      <Modal isOpen={showPasswordModal} onClose={() => setShowPasswordModal(false)} title="Generated Login Credentials" size="sm">
        <div className="space-y-4">
          <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl">
            <p className="text-sm text-amber-700 font-medium">⚠️ Save this password — it will only be shown once!</p>
          </div>
          <div className="space-y-2">
            <div className="p-3 bg-gray-50 rounded-lg">
              <p className="text-xs text-gray-500">Email</p>
              <p className="font-mono font-semibold text-sm">{selected?.email}</p>
            </div>
            <div className="p-3 bg-gray-50 rounded-lg">
          ...
          <p className="text-xs text-gray-500">An email with these credentials has been sent to {selected?.email}</p>
...
const AdmissionsView = () => {
  ...
  const filtered = data.students.filter(s => {
    ...
    return (tab === "All" || s.status === tab) &&
      (!q || name.includes(q) || s.admissionNo?.toLowerCase().includes(q));
  });
  ...
  const handleApprove = (student) => {
    ...
    setData(d => ({ ...d, students: d.students.map(s => s._id === student._id ? { ...s, status: 'Approved', rollNumber: nextRoll } : s) }));
  ...
  const handleReject = () => {
    ...
    setData(d => ({ ...d, students: d.students.map(s => s._id === selected._id ? { ...s, status: 'Rejected', rejectionRemark: remark } : s) }));
    showToast("Admission rejected", "error"); setShowRejectModal(false); setSelected(null); setRemark("");
  };

  const handleHold = () => {
    if (!remark) return showToast("Please enter hold reason", "error");
    setData(d => ({ ...d, students: d.students.map(s => s._id === selected._id ? { ...s, status: 'OnHold', holdRemark: remark } : s) }));
    showToast("Admission put on hold", "warning"); setShowHoldModal(false); setSelected(null); setRemark("");
  };
    ...
    return (
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <Card className="p-5 lg:col-span-2">
            <Tabs tabs={[{value:'student',label:'Student'},{value:'family',label:'Family'},{value:'docs',label:'Documents'}]} activeTab="student" onChange={() => {}} />
            <div className="mt-4 grid grid-cols-2 gap-3">
              {[["Father", selected.fatherName], ["Mother", selected.motherName], ["Father Phone", selected.fatherPhone], ["Mother Phone", selected.motherPhone], ["Father Email", selected.fatherEmail], ["Address", selected.residentialAddress]].map(([k, v]) => (
        ...
        <Modal isOpen={showRejectModal} onClose={() => setShowRejectModal(false)} title="Reject Admission" size="sm">
          <Textarea label="Rejection Reason *" value={remark} onChange={e => setRemark(e.target.value)} placeholder="Enter reason for rejection..." />
          <div className="flex gap-3 justify-end mt-4">
            <Button variant="outline" onClick={() => setShowRejectModal(false)}>Cancel</Button>
            <Button variant="danger" onClick={handleReject}>Reject Admission</Button>
          </div>
        </Modal>
        <Modal isOpen={showHoldModal} onClose={() => setShowHoldModal(false)} title="Put On Hold" size="sm">
          <Textarea label="Hold Reason *" value={remark} onChange={e => setRemark(e.target.value)} placeholder="Enter reason for hold..." />
          <div className="flex gap-3 justify-end mt-4">
            <Button variant="outline" onClick={() => setShowHoldModal(false)}>Cancel</Button>
            <Button variant="warning" onClick={handleHold}>Put On Hold</Button>
          </div>
        </Modal>
      </div>
    );
...
const StudentsView = ({ userRole }) => {
  ...
  const filtered = data.students.filter(s => {
    const q = search.toLowerCase();
    const name = `${s.firstName} ${s.lastName}`.toLowerCase();
    return (!q || name.includes(q) || s.admissionNo?.includes(q)) &&
  ...
  const handleMarkLeft = () => {
    ...
    setData(d => ({ ...d, students: d.students.map(s => s._id === selected._id ? { ...s, status: 'Left', ...leaveData } : s) }));
    showToast("Student marked as left. TC can be generated.", "success");
    setShowLeaveModal(false); setSelected(null);
  };
      ...
      <div className="space-y-5">
        <PageHeader title={`${selected.firstName} ${selected.lastName}`} breadcrumbs={["Home", "Students", selected.admissionNo]}
          actions={
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => setSelected(null)}>← Back</Button>
              {userRole === 'admin' && selected.status === 'Approved' && (
                <Button variant="danger" size="sm" onClick={() => setShowLeaveModal(true)}>🚪 Mark as Left</Button>
              )}
            </div>
          } />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-5">
          <Card className="p-5">
            ...
            <div className="mt-4 space-y-2 pt-4 border-t border-gray-100">
              {[["Adm No", selected.admissionNo], ["DOB", fmt(selected.dateOfBirth)], ["Gender", selected.gender], ["Blood Group", selected.bloodGroup || '-'], ["Aadhaar", selected.aadhaarNo || '-']].map(([k, v]) => (
          ...
          <div className="lg:col-span-3 space-y-4">
            <Tabs tabs={[{value:'info',label:'Info'},{value:'family',label:'Family'},{value:'fees',label:'Fees'},{value:'marks',label:'Marks'},{value:'attendance',label:'Attendance'}]}
              activeTab={detailTab} onChange={setDetailTab} />
            
            {detailTab === 'info' && (
              <Card className="p-5">
                <h4 className="font-bold text-gray-900 mb-4">Student Information</h4>
                <div className="grid grid-cols-2 gap-3 text-sm">
                  {[["Place of Birth", selected.placeOfBirth], ["Religion", selected.religion], ["Caste", selected.caste], ["Mother Tongue", selected.motherTongue], ["PEN Number", selected.penNumber], ["Previous School", selected.previousSchoolName]].map(([k, v]) => (
            ...
            {detailTab === 'family' && (
              <Card className="p-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <h4 className="font-bold text-gray-900 mb-3">👨 Father's Details</h4>
                    <div className="space-y-2 text-sm">
                      {[["Name", selected.fatherName], ["Occupation", selected.fatherOccupation], ["Phone", selected.fatherPhone], ["Email", selected.fatherEmail]].map(([k,v]) => (
                        <div key={k}><p className="text-xs text-gray-400">{k}</p><p className="font-medium">{v || '-'}</p></div>
                      ))}
                    </div>
                  </div>
                  <div>
                    <h4 className="font-bold text-gray-900 mb-3">👩 Mother's Details</h4>
                    <div className="space-y-2 text-sm">
                      {[["Name", selected.motherName], ["Occupation", selected.motherOccupation], ["Phone", selected.motherPhone], ["Email", selected.motherEmail]].map(([k,v]) => (
                        <div key={k}><p className="text-xs text-gray-400">{k}</p><p className="font-medium">{v || '-'}</p></div>
                      ))}
                    </div>
                  </div>
            ...
            {detailTab === 'fees' && (
              <Card className="p-5">
                <h4 className="font-bold text-gray-900 mb-4">Fee History</h4>
                <Table columns={[
                ...
                ]} data={studentFees} emptyMsg="No fee records" />
              </Card>
            )}
            {detailTab === 'marks' && (
              <Card className="p-5">
                <h4 className="font-bold text-gray-900 mb-4">Marks & Results</h4>
                <Table columns={[
                  { header: "Subject", render: r => { const s = data.subjects.find(s=>s._id===r.subjectId); return s?.name || '-'; } },
                  { header: "Exam", render: r => { const e = data.exams.find(e=>e._id===r.examId); return e?.name || '-'; } },
                  { header: "Marks", render: r => r.isAbsent ? <span className="text-red-500 font-medium">Absent</span> : `${r.marksObtained}/${data.exams.find(e=>e._id===r.examId)?.totalMarks || '-'}` },
                  { header: "Grade", render: r => <span className={`font-bold ${r.grade === 'F' ? 'text-red-500' : r.grade?.includes('A') ? 'text-green-600' : 'text-amber-600'}`}>{r.grade}</span> },
                  ...
                  return (
                    <div>
                      <div className="grid grid-cols-4 gap-3 mb-4">
                        {[["Total Days", total], ["Present", present], ["Absent", att.filter(a=>a.status==='Absent').length], ["Attendance %", `${pct}%`]].map(([k,v]) => (
                      ...
                      ]} data={att} emptyMsg="No attendance records" />
                    </div>
                  );
                })()}
              </Card>
        ...
        <Modal isOpen={showLeaveModal} onClose={() => setShowLeaveModal(false)} title="Mark Student as Left" size="sm">
          <div className="space-y-3">
            ...
            <Textarea label="Reason for Leaving *" value={leaveData.leavingReason} onChange={e => setLeaveData(p => ({...p, leavingReason: e.target.value}))} />
            <div className="flex gap-3 justify-end">
              <Button variant="outline" onClick={() => setShowLeaveModal(false)}>Cancel</Button>
              <Button variant="danger" onClick={handleMarkLeft}>Confirm & Generate TC</Button>
            </div>
          </div>
        </Modal>
      </div>
...
const EmployeesView = () => {
  ...
  const filtered = data.employees.filter(e => {
    const q = search.toLowerCase();
    return (!q || e.name?.toLowerCase().includes(q) || e.employeeId?.includes(q) || e.email?.toLowerCase().includes(q)) &&
  ...
  const handleCreate = () => {
    ...
    showToast(`${emp.name} added successfully! Employee ID: ${emp.employeeId}`, "success");
    ...
    return (
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-5">
          ...
          <div className="lg:col-span-3 space-y-4">
            <Tabs tabs={[{value:'info',label:'Personal'},{value:'classes',label:'Classes'},{value:'payroll',label:'Payroll'},{value:'leaves',label:'Leaves'}]}
              activeTab={empTab} onChange={setEmpTab} />
            {empTab === 'info' && (
              <Card className="p-5">
                <div className="grid grid-cols-2 gap-3 text-sm">
                  {[["Gender", selected.gender], ["DOB", fmt(selected.dateOfBirth)], ["Education", selected.education || '-'], ["Experience", selected.experience || '-'], ["Religion", selected.religion || '-'], ["Address", selected.homeAddress || '-']].map(([k,v]) => (
                    <div key={k}><p className="text-xs text-gray-400">{k}</p><p className="font-medium text-gray-900">{v}</p></div>
                  ))}
                </div>
              </Card>
            ...
            {empTab === 'classes' && (
              <Card className="p-5">
                <h4 className="font-bold text-gray-900 mb-4">Assigned Classes & Subjects</h4>
                <div className="space-y-2">
                  {empClasses.map(c => <div key={c._id} className="p-3 bg-indigo-50 rounded-xl text-sm font-medium text-indigo-700">🏫 Class Teacher: {c.displayName}</div>)}
                  {empSubjects.map(s => { const c = classes.find(cl=>cl._id===s.classId); return <div key={s._id} className="p-3 bg-emerald-50 rounded-xl text-sm font-medium text-emerald-700">📚 {s.name} — {c?.displayName}</div>; })}
                  {empClasses.length + empSubjects.length === 0 && <p className="text-gray-400 text-sm">No assignments yet</p>}
                </div>
              </Card>
            )}
            {empTab === 'payroll' && (
              <Card className="p-5">
                <h4 className="font-bold text-gray-900 mb-4">Payroll History</h4>
                <Table columns={[
                  { header: "Month", render: r => `${MONTHS[r.month-1]} ${r.year}` },
                  { header: "Basic", render: r => fmtCurrency(r.basicSalary) },
                  { header: "Present", key: "daysPresent" },
                  { header: "Absent", key: "daysAbsent" },
                  { header: "Deductions", render: r => fmtCurrency(r.deductions) },
                  { header: "Net Salary", render: r => <span className="font-bold text-emerald-600">{fmtCurrency(r.netSalary)}</span> },
                  { header: "Status", render: r => <Badge status={r.status} /> },
                ]} data={empPayroll} emptyMsg="No payroll records" />
              </Card>
            )}
            {empTab === 'leaves' && (
              <Card className="p-5">
                <h4 className="font-bold text-gray-900 mb-4">Leave History</h4>
                <Table columns={[
                  ...
                  { header: "Status", render: r => <Badge status={r.status} /> },
                ]} data={empLeaves} emptyMsg="No leave records" />
              </Card>
            )}
          </div>
        </div>
    ...
    <div className="space-y-5">
      ...
      <Card>
        <Table columns={[
          ...
          { header: "Status", render: r => <Badge status={r.status} /> },
          { header: "Actions", render: r => <Button size="sm" variant="outline" onClick={() => setSelected(r)}>View</Button> },
        ]} data={filtered} onRowClick={setSelected} emptyMsg="No employees found" />
      </Card>
...
const ClassroomsView = () => {
  ...
  const CLASS_NAMES = [
    { name: "Playgroup", order: 0 }, { name: "Nursery", order: 1 }, { name: "Jr. KG", order: 2 },
    { name: "Sr. KG", order: 3 }, { name: "Class 1", order: 4 }, { name: "Class 2", order: 5 },
    { name: "Class 3", order: 6 }, { name: "Class 4", order: 7 }, { name: "Class 5", order: 8 },
    { name: "Class 6", order: 9 }, { name: "Class 7", order: 10 }, { name: "Class 8", order: 11 },
  ];
  ...
  const handleCreate = () => {
    const selected = CLASS_NAMES.find(c => c.name === form.className);
    ...
    const cls = {
      _id: `cls${Date.now()}`, displayName, ...form, order: selected?.order || 99,
      isActive: true, academicYear: "ay1", monthlyFees: Number(form.monthlyFees)
    };
    setData(d => ({ ...d, classrooms: [...d.classrooms, cls] }));
  ...
  return (
    <div className="space-y-5">
          ...
          return (
            <Card key={cls._id} className="p-5 hover:shadow-md transition-shadow">
              ...
              <div className="mt-3 pt-3 border-t border-gray-100 grid grid-cols-2 gap-2 text-xs">
                ...
                {cls.capacity && <div><p className="text-gray-400">Capacity</p><p className="font-bold text-gray-900 text-sm">{cls.capacity}</p></div>}
              </div>
            </Card>
          );
        })}
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="Add New Classroom" size="md">
        <div className="space-y-4">
          <Select label="Class Name *" value={form.className} onChange={e => setForm(p => ({...p, className: e.target.value}))}
            options={CLASS_NAMES.map(c => ({value:c.name, label:c.name}))} />
          <Select label="Section" value={form.section} onChange={e => setForm(p => ({...p, section: e.target.value}))}
            options={[{value:'',label:'No Section'},{value:'A',label:'A'},{value:'B',label:'B'},{value:'C',label:'C'},{value:'D',label:'D'}]} />
          ...
          <Select label="Class Teacher" value={form.classTeacherId} onChange={e => setForm(p => ({...p, classTeacherId: e.target.value}))}
...
const SubjectsView = () => {
  ...
  const handleAssign = () => {
    ...
    const valid = rows.every(r => r.name && r.totalMarks);
    if (!valid) return showToast("Fill all subject details", "error");
    const newSubs = rows.map(r => ({
      _id: `sub${Date.now()}${Math.random()}`, name: r.name, classId: selClass,
      teacherId: r.teacherId, totalMarks: Number(r.totalMarks), academicYear: "ay1", isActive: true
    }));
    setData(d => ({ ...d, subjects: [...d.subjects, ...newSubs] }));
    showToast(`${newSubs.length} subject(s) assigned!`, "success");
        ...
        <Table columns={[
          ...
          { header: "Actions", render: r => (
            <div className="flex gap-1">
              <button onClick={() => { setData(d => ({ ...d, subjects: d.subjects.map(s => s._id === r._id ? {...s, isActive: !s.isActive} : s) })); }} className="text-xs px-2 py-1 rounded border border-gray-200 hover:bg-gray-50 text-gray-600">Toggle</button>
            </div>
          )},
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-12 gap-2 text-xs font-semibold text-gray-500 uppercase">
              ...
              <div className="col-span-3">Assign Teacher</div>
            ...
            {rows.map((row, i) => (
              <div key={i} className="grid grid-cols-12 gap-2 items-end">
                ...
                <div className="col-span-1 flex justify-center">
                  {rows.length > 1 && <button onClick={() => setRows(r => r.filter((_,j) => j !== i))} className="text-red-400 hover:text-red-600">✕</button>}
                </div>
              </div>
            ))}
          </div>
...
const ExamsView = ({ userRole }) => {
  ...
  const handleCreateExam = () => {
    ...
    setData(d => ({ ...d, exams: [...d.exams, exam] }));
  ...
  const loadMarksEntry = () => {
    ...
    setMarksData(classStudents.map(s => {
      const existing = existingMarks.find(m => m.studentId === s._id);
      return { studentId: s._id, student: s, marksObtained: existing?.marksObtained || "", isAbsent: existing?.isAbsent || false, remarks: existing?.remarks || "" };
    }));
  };
  ...
  const saveMarks = () => {
    ...
    const newMarks = marksData.map(m => ({
      _id: `mrk${Date.now()}${Math.random()}`, examId: marksExam, studentId: m.studentId,
    ...
    setData(d => {
      const filtered = d.marks.filter(m => m.examId !== marksExam);
      return { ...d, marks: [...filtered, ...newMarks] };
    });
    ...
    <div className="space-y-5">
      ...
      {tab === 'exams' && (
        <Card>
          ...
          ]} data={filteredExams} emptyMsg="No exams created yet" />
        </Card>
      )}
            ...
            <Card>
              <div className="p-4 border-b border-gray-100 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500">Out of {data.exams.find(e=>e._id===marksExam)?.totalMarks} marks</p>
                </div>
                <div className="flex gap-2">
                  <Button size="sm" variant="secondary" onClick={() => setMarksData(m => m.map(x => ({...x, isAbsent: false})))}>Mark All Present</Button>
                  ...
                  <tbody>
                    {marksData.map((m, i) => (
                      <tr key={i} className="border-b border-gray-50">
                        ...
                        <td className="px-4 py-2"><input type="checkbox" checked={m.isAbsent} onChange={e => setMarksData(d => d.map((x,j) => j===i ? {...x, isAbsent:e.target.checked, marksObtained:e.target.checked?'':x.marksObtained} : x))} className="w-4 h-4 text-red-500" /></td>
        ...
        <Card className="p-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {data.exams.slice(0, 6).map(exam => {
              ...
              const avg = examMarks.length > 0 ? Math.round(examMarks.filter(m=>!m.isAbsent).reduce((s,m) => s + m.marksObtained, 0) / Math.max(1, examMarks.filter(m=>!m.isAbsent).length)) : 0;
              return (
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="Create New Exam" size="md">
        <div className="space-y-4">
          ...
          <Select label="Exam Type *" value={form.examType} onChange={e => setForm(p => ({...p, examType: e.target.value, name: EXAM_TYPES.find(t=>t.value===e.target.value)?.label || ''}))}
            options={EXAM_TYPES} />
          ...
          <div className="space-y-1">
            ...
            {form.classId && form.subjectId && (
              <p className="text-xs text-indigo-600">Available marks for this subject: {getAvailableMarks(form.classId, form.subjectId)}</p>
            )}
          </div>
...
const AttendanceView = ({ type = "student" }) => {
  ...
  const saveAttendance = () => {
    if (type === 'student') {
      const records = attData.map(a => ({ _id: `att${Date.now()}${Math.random()}`, studentId: a.id, classId: selClass, date: selDate, status: a.status, remark: a.remark, academicYear: "ay1" }));
      setData(d => {
        const filtered = d.attendance.filter(a => !(a.classId === selClass && a.date === selDate));
        return { ...d, attendance: [...filtered, ...records] };
      });
    } else {
      const records = attData.map(a => ({ _id: `eatt${Date.now()}${Math.random()}`, employeeId: a.id, date: selDate, status: a.status, remark: a.remark, academicYear: "ay1" }));
      setData(d => ({...d, employeeAttendance: [...(d.employeeAttendance||[]).filter(a => a.date !== selDate), ...records]}));
    }
    setSaved(true);
    ...
    <div className="space-y-5">
      ...
      {attData.length > 0 && (
        <Card>
          <div className="p-4 border-b border-gray-100 flex items-center justify-between">
            <div>
              <h4 className="font-bold text-gray-900">Attendance for {fmt(selDate)}</h4>
              <p className="text-xs text-gray-500">{attData.length} {type === 'student' ? 'students' : 'employees'} · Present: {attData.filter(a=>a.status==='Present').length}</p>
            </div>
                  ...
                  <tr key={i} className="border-b border-gray-50 hover:bg-gray-50">
                    ...
                    {STATUS_OPTS.map(s => (
                      <td key={s} className="px-3 py-2 text-center">
                        <input type="radio" name={`att-${i}`} checked={a.status === s} onChange={() => setAttData(d => d.map((x,j) => j===i ? {...x, status:s} : x))} className={`w-4 h-4 ${s==='Present'?'accent-green-500':s==='Absent'?'accent-red-500':'accent-amber-500'}`} />
                      </td>
                    ))}
...
const FeesView = ({ userRole }) => {
  ...
  const filtered = data.fees.filter(fee => {
    ...
    return (!q || name.includes(q) || student?.admissionNo?.includes(q)) &&
      (!classFilter || fee.classId === classFilter) &&
      (!monthFilter || fee.month === Number(monthFilter)) &&
  ...
  const handleCollectFee = () => {
    ...
    setData(d => ({
      ...d,
      feePayments: [...d.feePayments, receipt],
      fees: d.fees.map(f => f._id === selFee._id ? {...f, status: Number(payForm.amountPaid) >= f.finalAmount ? 'Paid' : 'PartiallyPaid'} : f)
    }));
    showToast(`Payment recorded! Receipt: ${receipt.receiptNo}`, "success");
  ...
  return (
    <div className="space-y-5">
      ...
      {userRole !== 'parent' && (
        <Tabs tabs={[{value:'collect',label:'Fee Collection'},{value:'defaulters',label:'Defaulters'},{value:'reports',label:'Reports'}]} activeTab={tab} onChange={setTab} />
      )}
      ...
      {tab === 'defaulters' && (
        <Card>
          <div className="p-4 border-b border-gray-100">
            <h3 className="font-bold text-gray-900">Fee Defaulters</h3>
          </div>
          <Table columns={[
        ...
        <Card className="p-5">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={MONTHS.map((m, i) => ({
              ...
              pending: data.fees.filter(f => f.month === i+1 && f.status !== 'Paid').reduce((s,f) => s+f.finalAmount, 0),
            }))}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
        ...
        {selFee && (
          <div className="space-y-4">
            <div className="p-4 bg-gray-50 rounded-xl space-y-2 text-sm">
              {[["Tuition Fee", fmtCurrency(selFee.tuitionFee)], ["Transport Fee", fmtCurrency(selFee.transportFee)], ["Activity Fee", fmtCurrency(selFee.activityFee)], selFee.lateFine > 0 && ["Late Fine", fmtCurrency(selFee.lateFine)], selFee.discount > 0 && ["Discount", `-${fmtCurrency(selFee.discount)}`]].filter(Boolean).map(([k,v]) => (
            ...
            {payForm.paymentMode !== 'Cash' && (
              <Input label="Transaction ID" value={payForm.transactionId} onChange={e => setPayForm(p => ({...p, transactionId: e.target.value}))} />
            )}
...
const PayrollView = () => {
  ...
  const handleGenerate = () => {
    ...
    setData(d => ({ ...d, payroll: [...d.payroll, payroll] }));
    showToast("Payroll generated!", "success"); setShowGenModal(false);
  };

  const markPaid = (pr) => {
    setData(d => ({ ...d, payroll: d.payroll.map(p => p._id === pr._id ? {...p, status:'Paid', paymentDate: new Date().toISOString().split('T')[0]} : p) }));
              ...
              <div key={hw._id} className={`p-4 rounded-xl border ${isOverdue ? 'border-red-100 bg-red-50' : 'border-gray-100 bg-white'}`}>
                <div className="flex items-start justify-between flex-wrap gap-2">
                  ...
                  <div className="text-right text-xs">
                    ...
                    <p className={`font-semibold ${isOverdue ? 'text-red-500' : 'text-gray-700'}`}>{fmt(hw.dueDate)}</p>
                  </div>
                </div>
              </div>
            );
...
const TimetableView = ({ userRole }) => {
  const { data, getClassrooms } = useApp();
                      ...
                      <td key={day} className="px-4 py-3 text-center">
                        {sub ? (
                          <div className="p-2 bg-indigo-50 rounded-lg">
                            <p className="font-medium text-indigo-700 text-xs">{sub.name}</p>
                            <p className="text-xs text-gray-500">{teacher?.name?.split(' ')[0]}</p>
                          </div>
            ...
            <div className="flex gap-3">
              {['parent','teacher','admin'].map(r => (
                <label key={r} className="flex items-center gap-1 cursor-pointer">
                  <input type="checkbox" checked={(form.targetRoles||[]).includes(r)} onChange={e => setForm(p => ({...p, targetRoles: e.target.checked ? [...(p.targetRoles||[]), r] : (p.targetRoles||[]).filter(x => x !== r)}))} className="text-indigo-600" />
...
const LeavesView = ({ userRole }) => {
  ...
  const handleApply = () => {
    ...
    setData(d => ({ ...d, leaves: [...d.leaves, leave] }));
    showToast("Leave application submitted!", "success"); setShowModal(false);
    setForm({ leaveType: "Casual", fromDate: "", toDate: "", reason: "" });
  };

  const handleAction = () => {
    setData(d => ({ ...d, leaves: d.leaves.map(l => l._id === selLeave._id ? {...l, status: action, approvalRemark: remark, approvedBy: "usr2"} : l) }));
...
const PromoteView = () => {
  ...
  const handlePromote = () => {
    const updates = students.map(s => ({
      ...data.students.find(st => st._id === s._id),
    ...
    setData(d => ({ ...d, students: d.students.map(st => { const upd = updates.find(u => u._id === st._id); return upd || st; }) }));
    showToast(`Promotion complete! ${students.filter(s=>s.result==='Promoted').length} promoted, ${students.filter(s=>s.result==='Detained').length} detained, ${students.filter(s=>s.result==='Left').length} left`, "success");
    ...
    <div className="space-y-5">
      ...
      <Card className="p-5">
        ...
        <div className="flex gap-3 items-end">
          <div className="flex-1">
            <Select label="From Class" value={selClass} onChange={e => setSelClass(e.target.value)} options={classes.map(c => ({value:c._id, label:c.displayName}))} />
          </div>
      ...
      {students.length > 0 && (
        <>
          <Card>
            <div className="p-4 border-b border-gray-100 flex items-center justify-between">
              ...
              <div className="flex gap-2 text-xs">
                ...
                <span className="px-2 py-1 bg-gray-100 text-gray-700 rounded-full">Left: {students.filter(s=>s.result==='Left').length}</span>
              </div>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {students.map((s, i) => (
                    <tr key={s._id} className="border-b border-gray-50 hover:bg-gray-50">
                      ...
                      <td className="px-4 py-2"><input value={s.remark} onChange={e => setStudents(d => d.map((x,j) => j===i ? {...x, remark:e.target.value} : x))} className="w-32 px-2 py-1 border border-gray-200 rounded text-sm" placeholder="Optional" /></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <Confirm isOpen={showConfirm} onClose={() => setShowConfirm(false)} onConfirm={handlePromote}
            title="Confirm Promotion" message={`This will promote ${students.filter(s=>s.result==='Promoted').length} students. This action cannot be easily undone. Are you sure?`}
...
const SettingsView = () => {
  ...
  const handleSave = () => {
    setData(d => ({ ...d, schoolSettings: form }));
  ...
  return (
    <div className="space-y-5">
      ...
      <Tabs tabs={[{value:'school',label:'School Info'},{value:'academic',label:'Academic Year'},{value:'fees',label:'Fee Settings'},{value:'grades',label:'Grade Settings'}]} activeTab={tab} onChange={setTab} />
      ...
      {tab === 'school' && (
        <Card className="p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2"><Textarea label="Declaration Text (for admission form)" value={form.declaration} onChange={e => setForm(p => ({...p, declaration: e.target.value}))} rows={3} /></div>
          </div>
          ...
          <div className="space-y-3">
            {data.academicYears.map(ay => (
              <div key={ay._id} className={`flex items-center justify-between p-4 rounded-xl border ${ay.isCurrent ? 'border-indigo-200 bg-indigo-50' : 'border-gray-100'}`}>
                <div>
                  <p className="font-semibold text-gray-900">{ay.name}</p>
                  <p className="text-xs text-gray-500">{fmt(ay.startDate)} — {fmt(ay.endDate)}</p>
                </div>
      ...
      {tab === 'fees' && (
        <Card className="p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><Input label="Late Fine Per Day (₹)" type="number" value={form.lateFinePer} onChange={e => setForm(p => ({...p, lateFinePer: Number(e.target.value)}))} /></div>
            <div><Input label="Fee Due Day (of each month)" type="number" value={form.feeDueDay} onChange={e => setForm(p => ({...p, feeDueDay: Number(e.target.value)}))} min={1} max={31} /></div>
          </div>
          <div className="mt-4"><Button onClick={handleSave}>Save Settings</Button></div>
        </Card>
      )}
      ...
      {tab === 'grades' && (
        <Card className="p-6">
          <h3 className="font-bold text-gray-900 mb-4">Grade Scale</h3>
          <div className="space-y-2">
            {[['A+', '≥ 90%'], ['A', '≥ 80%'], ['B+', '≥ 70%'], ['B', '≥ 60%'], ['C', '≥ 50%'], ['D', '≥ 40%'], ['F', '< 40%']].map(([g, range]) => (
              <div key={g} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <span className={`font-bold w-8 ${g === 'F' ? 'text-red-500' : g.includes('A') ? 'text-green-600' : 'text-amber-600'}`}>{g}</span>
                <span className="text-sm text-gray-500">{range}</span>
                <span className="w-8 h-5 rounded" style={{ background: g === 'A+' ? '#065f46' : g === 'A' ? '#10b981' : g === 'B+' ? '#78716c' : g === 'B' ? '#92400e' : g === 'C' ? '#b45309' : g === 'D' ? '#d97706' : '#ef4444' }} />
              </div>
            ))}
          </div>
        </Card>
...
const ReportsView = () => {
  ...
  const classwiseData = classes.map(c => ({
    ...
    present: data.attendance.filter(a => a.classId === c._id && a.status === 'Present').length,
  }));

  const attendanceTrend = MONTHS.slice(0, 6).map((m, i) => ({
    month: m, rate: 75 + Math.floor(Math.random() * 20)
  }));
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <Card className="p-5">
          <h3 className="font-bold text-gray-900 mb-4">Monthly Attendance Trend</h3>
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={attendanceTrend}>
              ...
              <Tooltip formatter={v => `${v}%`} />
              <Line type="monotone" dataKey="rate" stroke="#10B981" strokeWidth={2} dot={{ fill: '#10B981', r: 4 }} name="Attendance %" />
            </LineChart>
          </ResponsiveContainer>
        </Card>
      </div>
...
const ParentDashboard = () => {
  const { data, getClassrooms } = useApp();
  const myStudent = data.students.find(s => s.status === 'Approved' && s.userId === 'usr3') || data.students.find(s => s.status === 'Approved');
  const classes = getClassrooms();
  ...
  const upcomingHomework = data.homework.filter(hw => hw.classId === myStudent?.classId && new Date(hw.dueDate) >= new Date());
      ...
      {myStudent ? (
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <StatCard label="Pending HW" value={upcomingHomework.length} icon="📖" color="amber" />
                ...
                {data.homework.filter(hw => hw.classId === myStudent.classId).slice(0, 3).map(hw => {
                  ...
                  return (
                    <div key={hw._id} className={`p-3 rounded-xl ${overdue ? 'bg-red-50' : 'bg-gray-50'}`}>
                      <div className="flex items-center justify-between">
                        <div>
                          <span className="text-xs px-2 py-0.5 bg-indigo-100 text-indigo-700 rounded-full">{sub?.name}</span>
                          <p className="font-medium text-sm mt-1">{hw.title}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-xs text-gray-400">Due</p>
                          <p className={`text-xs font-semibold ${overdue ? 'text-red-500' : 'text-gray-700'}`}>{fmt(hw.dueDate)}</p>
                        </div>
                      </div>
                    </div>
                  );
        ...
        <Card className="p-8 text-center">
          ...
          <h3 className="text-lg font-bold text-gray-900 mb-2">No Active Student Found</h3>
          <p className="text-gray-500 text-sm">Your child's admission may be under review or pending. Contact the school for more information.</p>
        </Card>
      )}
    </div>
  );
...
const TeacherDashboard = () => {
  const { data, getClassrooms } = useApp();
  const myEmployee = data.employees.find(e => e.email === 'priya@school.edu') || data.employees[0];
  const classes = getClassrooms();
                  ...
                  return (
                    <div key={p.periodNo} className="flex items-center gap-3 p-3 bg-indigo-50 rounded-xl">
                      <div className="text-center w-14">
                        <p className="text-xs font-bold text-indigo-600">P{p.periodNo}</p>
                        <p className="text-xs text-gray-500">{p.startTime}</p>
                      </div>
                      <div>
                        <p className="font-medium text-sm text-gray-900">{sub?.name}</p>
                        <p className="text-xs text-gray-500">{cls?.displayName}</p>
                      </div>
                    </div>
                  );
                })}
...
const PrincipalDashboard = () => {
  const { data, getClassrooms } = useApp();
  const classes = getClassrooms();
  const totalStudents = data.students.filter(s => s.status === 'Approved').length;
  const totalTeachers = data.employees.filter(e => e.role === 'teacher' && e.status === 'active').length;
  const pendingLeaves = data.leaves.filter(l => l.status === 'Pending').length;
  const pendingAdm = data.students.filter(s => s.status === 'UnderReview').length;

  return (
    <div className="space-y-6">
      <PageHeader title="Principal Dashboard" subtitle="School Overview" breadcrumbs={["Home", "Dashboard"]} />
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total Students" value={totalStudents} icon="👨‍🎓" color="indigo" />
        <StatCard label="Total Teachers" value={totalTeachers} icon="👨‍🏫" color="green" />
        <StatCard label="Pending Leaves" value={pendingLeaves} icon="🏖️" color="amber" />
        <StatCard label="Pending Admissions" value={pendingAdm} icon="📋" color="red" />
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <Card className="p-5">
          <h3 className="font-bold text-gray-900 mb-4">Class-wise Students</h3>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={classes.map(c => ({ name: c.displayName.replace('Class ','C').replace('Jr. ','Jr.').replace('Sr. ','Sr.'), count: data.students.filter(s=>s.classId===c._id&&s.status==='Approved').length }))}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
              ...
              return emp ? (
                <div key={l._id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="font-medium text-sm">{emp.name}</p>
                      <p className="text-xs text-gray-500">{l.leaveType} · {l.totalDays} day(s)</p>
                    </div>
                  </div>
...
// --- Profile View for Parent's Child ---
const ProfileView = () => {
  const { data, getClassrooms } = useApp();
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <Card className="p-5 lg:col-span-2">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <h5 className="font-semibold text-gray-700 mb-2">👨 Father</h5>
              <p>{myStudent.fatherName}</p>
              <p className="text-gray-500">{myStudent.fatherPhone}</p>
              <p className="text-gray-500 text-xs">{myStudent.fatherEmail}</p>
            </div>
            <div>
              <h5 className="font-semibold text-gray-700 mb-2">👩 Mother</h5>
              <p>{myStudent.motherName}</p>
              <p className="text-gray-500">{myStudent.motherPhone}</p>
            </div>
          </div>
        </Card>
      </div>
...
export default function SchoolManagementSystem() {
  ...
  const setData = (updater) => setDataRaw(prev => typeof updater === 'function' ? updater(prev) : updater);
  ...
  const getClassrooms = () => [...data.classrooms].filter(c => c.isActive).sort((a, b) => a.order - b.order);
  ...
  const renderPage = () => {
    ...
    switch (currentPage) {
      ...
      case 'timetable': return <TimetableView userRole={role} />;
  ...
  if (!user) return (
    <AppContext.Provider value={{ data, setData, showToast, getClassrooms }}>
          ...
          <div className="p-5 border-b border-gray-100">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="font-bold text-gray-900 text-sm leading-tight truncate">Sunrise School</p>
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0">
          ...
          <header className="bg-white border-b border-gray-100 px-4 py-3 flex items-center justify-between flex-shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-bold text-gray-900 capitalize text-sm">{currentPage.replace(/-/g,' ')}</h2>
                <p className="text-xs text-gray-400">{data.schoolSettings.name}</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <div className="relative">
          ...
          <nav className="lg:hidden bg-white border-t border-gray-100 px-2 py-1 flex justify-around flex-shrink-0">
            {(navConfig[0]?.items?.slice(0,1) || []).concat(navConfig.slice(1, 4).map(s => s.items[0]).filter(Boolean)).slice(0,5).map((item, i) => (
              <button key={item?.id || i} onClick={() => item && setCurrentPage(item.id)}
                className={`flex flex-col items-center p-2 rounded-xl min-w-0 ${currentPage === item?.id ? 'text-indigo-600' : 'text-gray-400'}`}>
